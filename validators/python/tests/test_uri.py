...
def test_uri_parse_exception():
    """Test URI parsing exception handling."""
    validator = URIValidator()
    # Test with completely malformed URI that might cause urlparse to fail
    result = validator.validate("ajson://[invalid")
    
    # Should handle gracefully with error
    assert not result.is_valid


def test_uri_invalid_port_number():
    """Test URI with invalid port number (out of range)."""
    validator = URIValidator()
    result = validator.validate("ajson://example.com:99999/agents/test")
    
    assert not result.is_valid
    assert any("port" in error.lower() for error in result.errors)


def test_uri_invalid_port_string():
    """Test URI with non-numeric port."""
    validator = URIValidator()
    result = validator.validate("ajson://example.com:abc/agents/test")
    
    assert not result.is_valid
    assert any("port" in error.lower() for error in result.errors)


def test_uri_invalid_fragment_chars():
    """Test URI with invalid characters in fragment."""
    validator = URIValidator()
    result = validator.validate("ajson://example.com/agents/test#invalid space")
...
def test_uri_with_query_params():
    """Test URI with query parameters (should warn)."""
    validator = URIValidator()
    result = validator.validate("ajson://example.com/agents/test?param=value")
